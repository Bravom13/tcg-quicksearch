...
const fetchAndProcessCardData = async (cardName, page)=>{
    ...
    try {
        ...
        if (sortByNewest) idParams.orderBy = '-set.releaseDate';
...
const renderPrices = (prices)=>{
    ...
    const priceList = Object.entries(prices).map(([key, value])=>{
        if (value && value.market) return `<li><strong>${key}:</strong> ${value.market.toFixed(2)}</li>`;
        return '';
    }).join('');
    return `
...
const displayRequestTime = (duration, fromCache = false)=>{
    if (fromCache) diagnosticsContainer.innerHTML = 'Request Time: <span style="color: green;">Served from cache</span>';
    else diagnosticsContainer.innerHTML = `Request Time: ${duration.toFixed(2)} ms`;
};
// --- Event Listeners ---
...
},{"./style.css":"bhJkM"}],"bhJkM":[function() {},{}]},["ieapP","a0t4e"], "a0t4e", "parcelRequire202e", {})

//# sourceMappingURL=tcg-quicksearch.31b563d9.js.map
